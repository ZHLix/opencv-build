...
import { formatNumber, highlight, protect, spawn, toExecCmd } from './utils'
...
export class SetupOpencv {
  ...
  public async updateOpencvRawDownloadPath() {
    const env = this.builder.env
    const files = [
  ...
  public async start(): Promise<void> {
    ...
    if (env.isWin) {
      if (!msbuild) throw Error('Error getting Ms Build info')
    ...
    try {
      ...
      for (const dir of dirs) await primraf(dir)
