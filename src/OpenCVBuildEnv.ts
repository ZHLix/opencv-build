...
export default class OpenCVBuildEnv implements OpenCVBuildEnvParamsBool, OpenCVBuildEnvParamsString {
  ...
  private resolveValue(info: ArgInfo): string {
    if (info.conf in this.opts) {
      ...
      } else return (this.opts[info.conf] as string) || ''
    } else {
  ...
  constructor(private opts = {} as OpenCVBuildEnvParams) {
    ...
    if (!this.no_autobuild && opts.prebuild) {
      ...
      if (!autoBuildFile) throw Error(`failed to read build info from ${builds[0].autobuild}`)
    ...
    if (this.no_autobuild) {
      ...
      if (os === 'win32') {
        ...
        if (!process.env.OPENCV_LIB_DIR) {
          ...
          if (candidates.length) process.env.OPENCV_LIB_DIR = candidates[0]
        }
        if (!process.env.OPENCV_INCLUDE_DIR) {
        ...
        if (candidates.length) {
          ...
          if (!process.env.OPENCV_LIB_DIR) {
            ...
            if (fs.existsSync(candidate)) process.env.OPENCV_LIB_DIR = candidates[0]
          }
          if (!process.env.OPENCV_INCLUDE_DIR) {
    ...
    const envKeys: (keyof OpenCVPackageBuildOptions)[] = Object.keys(this.#packageEnv) as any[]
    if (envKeys.length) {
      ...
      envKeys.forEach(key => log.info('applyEnvsFromPackageJson', `${highlight(key)}: ${formatNumber(this.#packageEnv[key] || '')}`))
    }
  ...
  public enableModule(mod: OpencvModulesType) {
    if (this.#ready) throw Error('No mode modules change can be done after initialisation done.')
